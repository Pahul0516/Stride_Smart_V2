...
class Information:
    ...
    def calculate_green_index(self, edge, raster_src):
        ...
        if not edge_buffer.is_valid or edge_buffer.is_empty:
            return None
        shapes = [edge_buffer._geo_interface_]
...
class SaftyPath:
    ...
    def get_path(self,start_coords,goal_coords,alpha = 0.5, beta = 0.5):
        ...
        self.start_node = ox.distance.nearest_nodes(G, X=start_coords[1], Y=start_coords[0])  # X is longitude, Y is latitude
        self.goal_node = ox.distance.nearest_nodes(G, X=goal_coords[1], Y=goal_coords[0])
...
while 1:
    print("daca vrei ruta safe apasa 1")
    print("daca vrei ruta green apasa 2")
    print("daca vrei ruta accessibility  apasa 3")

    val = int(input())
    match val:
        case 1:
            line = input("Enter start coordinates separated by a comma:\n")
            parts = line.split(",")
            start_coords = (float(parts[0].strip()), float(parts[1].strip()))
            line = input("Enter goal coordinates separated by a comma:\n")
            parts = line.split(",")
            goal_coords = (float(parts[0].strip()), float(parts[1].strip()))
            line = input("Enter priorities first for lenght, second for safty route:\n")
            parts = line.split(",")
            path = b.get_path(start_coords,goal_coords,float(parts[0]), float(parts[1]))
            b.show()
        case 2:
            line = input("Enter start coordinates separated by a comma:\n")
            parts = line.split(",")
            ...
            parts = line.split(",")
            goal_coords = (float(parts[0].strip()), float(parts[1].strip()))
            line = input("Enter priorities first for lenght, second for green route:\n")
            parts = line.split(",")
            path = c.get_path(start_coords, goal_coords,float(parts[0]), float(parts[1]))
            c.show()
        case 3:
            line = input("Enter start coordinates separated by a comma:\n")
            parts = line.split(",")
